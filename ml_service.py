...
Loads the trained sentiment pipeline (`sentiment_naive_bayes.pkl`, built by
the ML teammate) and exposes prediction/text-utility helpers consumed by the
routers. The pkl is a single scikit-learn `Pipeline` (TfidfVectorizer +
MultinomialNB), so no separate vectorizer file is needed -- it accepts raw
text directly.
"""
import re
import string
